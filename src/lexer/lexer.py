    ...
    def analyse(self, eof_flag=False):
        ...
        if eof_flag:
            if len(input_char_buffer) > 0:
                self.output_queue.append((Token.ERR_TOKEN, ''.join(input_char_buffer)))
        ...
        elif len(input_char_buffer) > 0:
            self.input_queue.append(''.join(input_char_buffer))
